  ...
  function filtrarLivros() {
    if (usuario) {
      if (statusLidos && statusNaoLidos) {
        ...
        getLivrosStatus(usuario.uid, true).then((resultados) =>
          setLivros(resultados)
        );
      } else if (statusNaoLidos) {
        getLivrosStatus(usuario.uid, false).then((resultados) =>
          setLivros(resultados)
        );
      } else {
        carregarDados();
  ...
  useEffect(() => {
    if (usuario) {
      if (categoriaSelecionada === "Todos") {
        ...
        getLivrosCategoria(usuario.uid, categoriaSelecionada).then(
      ...
      <div className="m-3 d-flex justify-content-evenly align-items-center gap-3 filter-container">
        <div className="d-flex align-items-center gap-2">
          <div>
            ...
            />
            <label htmlFor="lidos" className="ms-2 form-check-label">
              Lidos
            </label>
          </div>
          <div>
            <input
              ...
              className="form-check-input"
            />
            <label htmlFor="nao-lidos" className="ms-2 form-check-label">
              Não Lidos
            </label>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-center">
          <Link to="/livros/adicionar" className="btn btn-outline-light">
            Adicionar Livro
          </Link>
        </div>
      </div>
              ...
              >
                <Card className="h-100 card-custom card-livros">
                  <Card.Body>
                    ...
                    <Card.Text>
                      <span className="fw-bold">Autor:</span> {livro.autor}
                    </Card.Text>
                    <Card.Text>
                    ...
                    <div className="mb-2 mt-4">
                      ...
                      {livro.lido ? (
                        <Badge bg="success">Lido</Badge>
                      ) : (
                        <Badge bg="warning">Não Lido</Badge>
                      )}
                    </div>
